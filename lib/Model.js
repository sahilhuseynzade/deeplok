...
function activeBlock(state, nowMs) {
  var now = new Date(nowMs)
  var items = []   // { name, endsAt, locked, blocklistIds }
  var i
  for (i = 0; i < state.sessions.length; i++) {
    var ses = state.sessions[i]
    if (!sessionActiveAt(ses, nowMs)) continue
    items.push({ name: "Session", endsAt: ses.endsAt, locked: ses.locked, blocklistIds: ses.blocklistIds })
  }
  for (i = 0; i < state.schedules.length; i++) {
    var sch = state.schedules[i]
    if (!scheduleActiveAt(sch, now)) continue
    items.push({ name: sch.name, endsAt: scheduleEndMs(sch, now), locked: sch.locked, blocklistIds: sch.blocklistIds })
  }
