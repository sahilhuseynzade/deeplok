...
// Change-detection signature for the engine: hosts content, the lock
// horizon, and the block's end time (shown on the hosted block page).
// When this string is unchanged, no helper call is needed.
function applySignature(block) {
  return hostsDomains(block.domains).join(",")
    ...
    + "|" + Math.floor((block.endsAt || 0) / 1000)
}
...
function applyPayload(block) {
  return JSON.stringify({
    ...
    endsAt: Math.floor((block.endsAt || 0) / 1000)
  })
}
